        ...
        <nav class="mt-2 px-4 pb-4 space-y-1">
            ...
            @can('viewAny', App\Domains\Patient\Models\Patient::class)
                <a href="{{ route('patients.index') }}" wire:navigate
                    class="flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg transition-colors group {{ request()->routeIs('patients.index') ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @can('viewAny', App\Domains\Patient\Models\Patient::class)
                <a href="{{ route('patients.clinical.monitor') }}" wire:navigate
                    ...
                    <span x-show="isSidebarExpanded" class="ml-auto flex h-2 w-2 relative">
                        <span
